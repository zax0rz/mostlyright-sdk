...
must_haves:
  truths:
    - "All 4 npm packages (`@mostlyright/{core,weather,markets}` + unscoped meta `mostlyright`) published at vts-0.1.0 on npm with `latest` dist-tag"
    - "`npm install @mostlyright/core` in clean Node 20 + pnpm 9 project resolves to 0.1.0; smoke import works"
    ...
    - "scripts/release-ts-preflight.mjs Phase 12 iter-1 CRITICAL fix verified: peerDependencies['@mostlyright/core'] rewritten on each publish"
  ...
  artifacts:
    ...
    - path: ".planning/phases/14-npm-publication-pipeline/OPERATOR-PREFLIGHT.md"
      provides: "OP3 + OP4 checklist (`@mostlyright` scope + 4 OIDC publishers + 1 GH Environment)"
  key_links:
    - from: "git tag vts-*rc*"
...
<task type="auto" depends_on="">
  ...
  <action>
    ...
    Step 2 — commit:
      ...
      git commit -m "phase14 W1: operator pre-flight checklist for npm publishers + @mostlyright scope"
  </action>
  <verify>
    <automated>
...
<task type="auto" depends_on="W1-T1" wait_for="OPERATOR_PREFLIGHT_CONFIRMED">
  ...
  <verify>
    ...
    curl -fsS 'https://registry.npmjs.org/@mostlyright/core' | jq -r '."dist-tags".next' | grep -c '0.1.0-rc.1'  # expect 1
    curl -fsS 'https://registry.npmjs.org/@mostlyright/weather' | jq -r '."dist-tags".next' | grep -c '0.1.0-rc.1'  # expect 1
    </automated>
  </verify>
  <done>
...
<task type="auto" depends_on="W2-T1">
  ...
  <action>
    Step 1 — clean Node project:
      ...
      pnpm init -y
      pnpm add @mostlyright/core@next

    Step 2 — version + ESM import:
      node -e "import('@mostlyright/core').then(m => console.log(Object.keys(m).length))"
    ...
    Step 5 — browser smoke via examples/chrome-extension-mvp/:
      ...
      # Edit src to import from `@mostlyright/core` (NOT the in-repo workspace path);
      # publish the extension's package.json with a real dep `"@mostlyright/core": "next"` so pnpm install pulls from npm.
      pnpm install
  ...
  <done>
    Node 20 + pnpm 9 clean install of `@mostlyright/core@next` works; ESM + research() smoke pass. Browser smoke via Chrome MV3 SW + kalshi.com overlay works. Both transcripts committed.
  </done>
</task>
...
<task type="manual" depends_on="W2-T2">
  ...
  <action>
    ...
    Step 3 — log feedback in 14-W3-SOAK-LOG.md:
      ...
      - Quickstart wall-clock time (target: <5 min for `pnpm add @mostlyright/core@next` + first research() call returning data)
...
<task type="auto" depends_on="W3-T1">
  ...
  <action>
    ...
    Step 3 — verify release-ts-preflight.mjs locally (sanity — the workflow re-runs in CI):
      ...
      # Expected: 'Rewrote packages-ts/weather peerDependencies[@mostlyright/core]: ... → ^0.1.0' x2 (weather + markets), then 'Preflight green; safe to publish.'
  ...
  <verify>
    ...
    curl -fsS 'https://registry.npmjs.org/@mostlyright/core' | jq -r '."dist-tags".latest' | grep -c '^0.1.0$'  # expect 1
    curl -fsS 'https://registry.npmjs.org/@mostlyright/weather' | jq -r '."dist-tags".latest' | grep -c '^0.1.0$'  # expect 1
    curl -fsS 'https://registry.npmjs.org/@mostlyright/markets' | jq -r '."dist-tags".latest' | grep -c '^0.1.0$'  # expect 1
...
<task type="auto" depends_on="W4-T1">
  ...
  <action>
    Step 1 — clean Node project smoke install (NO @next, expects to resolve from `latest`):
      ...
      pnpm init -y
      pnpm add @mostlyright/core  # No @next; resolves to latest
      node -e "import('@mostlyright/core').then(m => console.log(Object.keys(m).length))"
    ...
    Step 2 — browser smoke from npm `latest`:
      Update packages-ts/examples/chrome-extension-mvp/package.json:
        "dependencies": { "@mostlyright/core": "latest" }  # was @next
      pnpm install
      pnpm run build
    ...
    Step 3 — write RUNBOOK.md:
      sections (mirror Phase 13 RUNBOOK.md):
        ...
        - Rollback / deprecate policy: `npm deprecate @mostlyright/core@0.1.0 "<reason>"` instead of unpublish (which fails after 24h grace window for non-private scopes)
        - Provenance verification: `npm view @mostlyright/core@0.1.0 --json | jq '.attestations'` shows the OIDC attestation block
...
<verification>
  <automated>
    ...
    curl -fsS 'https://registry.npmjs.org/@mostlyright/weather' | jq -r '."dist-tags".latest' | grep -q '^0.1.0$'
    curl -fsS 'https://registry.npmjs.org/@mostlyright/markets' | jq -r '."dist-tags".latest' | grep -q '^0.1.0$'
