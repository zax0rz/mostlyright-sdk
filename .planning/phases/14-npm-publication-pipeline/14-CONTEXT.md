...
Stand up the npm publish pipeline end-to-end for the 5 TypeScript packages (`@mostlyrightmd/core`, `@mostlyrightmd/weather`, `@mostlyrightmd/markets`, `@mostlyrightmd/codegen`, unscoped meta `mostlyright`), close out operator pre-flight OP3 + OP4 deferred from Phase 12, run a `vts-0.1.0rc1` `npm dist-tag next` dry-run, soak ≥1 week, and promote `vts-0.1.0` to npm `latest`.

**What this phase ships:**
- **Pre-condition: Phase 13 W0 must have shipped** — `mostlyrightmd` GitHub org exists, repo transferred to `mostlyrightmd/mostlyright-sdk`. All npm OIDC publishers below bind to the NEW repo coordinates.
- 1 npm scope claim: `@mostlyrightmd` (operator OP3) — **claimed UNDER the `mostlyrightmd` GitHub org** (linked to the org-owner npm account, not the individual `helloiamvu` account, so the scope ownership matches the repo ownership for future team handoffs).
- 4 npm OIDC pending-publisher registrations (operator OP4): `@mostlyrightmd/{core,weather,markets}` + unscoped meta `mostlyright`, all bound to `mostlyrightmd/mostlyright-sdk` + `release-ts.yml` + env `npm`. **`@mostlyrightmd/codegen` is NOT published** — it's `"private": true` in package.json (build-only).
- GH repo Environment `npm` on `mostlyrightmd/mostlyright-sdk` (operator-gated, GH UI, mirrors Python's `pypi` posture)
- `vts-0.1.0rc1` git tag pushed to `mostlyrightmd/mostlyright-sdk` → release-ts.yml fires → 4 packages on npm with `--tag next` dist-tag
- Clean Node 20 + pnpm 9 project smoke install: `npm install @mostlyrightmd/core@next`
- Browser smoke test: `packages-ts/examples/chrome-extension-mvp/` rebuilt against `@mostlyrightmd/core@next` IIFE bundle, loaded in Chrome MV3 SW
...
1. **`vts-0.1.0rc1`** → release-ts.yml `--tag next` → `npm install @mostlyrightmd/core@next`. Soak ≥1 week. External installer (not maintainer) confirms `npm install @mostlyrightmd/core@next` in clean Node 20 + pnpm 9 project + browser smoke via `examples/chrome-extension-mvp/` working.
2. **`vts-0.1.0`** (final) → release-ts.yml `--tag latest` → default `npm install @mostlyrightmd/core`. Soak ≥2 weeks. Real users install from default channel.
...
- OIDC trusted-publisher bindings on npmjs.com keyed to **(owner, repo, workflow filename, environment name, package name)** for each of the 4 publishers — changing ANY of those 5 invalidates the binding. Phase 12 W6 preserved workflow filename + env names. **Phase 13 W0 changed (owner, repo) from `helloiamvu/tradewinds` → `mostlyrightmd/mostlyright-sdk` via repo transfer.** npm publisher registrations therefore MUST be done AFTER the Phase 13 W0 transfer completes, against the new repo coordinates.
...
Users wanting to soak rc explicitly opt in via `npm install @mostlyrightmd/core@next`. Default `npm install @mostlyrightmd/core` resolves to `latest` dist-tag = the last non-rc publish.

### Changesets lockstep (LOCKED — fixed group of 4)

`.changeset/config.json` declares the fixed group: `["@mostlyrightmd/core", "@mostlyrightmd/weather", "@mostlyrightmd/markets", "mostlyright"]`. `@mostlyrightmd/codegen` is build-only + `"private": true` so it's not in the fixed group and doesn't publish.
...
Phase 12 review-iter1 fixed the script to rewrite `peerDependencies['@mostlyrightmd/core']` (not the old `@tradewinds/core` key). Phase 14 verifies the preflight still fires correctly on each tagged publish — if it silently no-ops again the published `@mostlyrightmd/weather` + `@mostlyrightmd/markets` would carry `peerDependencies['@mostlyrightmd/core']: '^0.0.0'` and break every consumer install under pnpm strict-peer-dependencies. The preflight emits a hard error if the key is missing.

### Operator pre-flight (REQUIRED before any in-repo wave executes)

Phase 12 closeout deferred OP3 + OP4. Phase 14 W1 documents + tracks them; no in-repo PR work proceeds until the operator confirms. **Pre-condition: Phase 13 W0 already closed (org created, repo transferred to `mostlyrightmd/mostlyright-sdk`).**

- **OP3** (REQUIRED): Claim `@mostlyrightmd` npm scope on npmjs.com **under the `mostlyrightmd` GitHub org's linked npm account** (the operator's npm account that owns the `mostlyright` GH org — same person, but scope ownership documented as org-linked so future team handoffs don't require scope transfer). Set scope to public access (mirrors PyPI public-by-default posture). If `@mostlyrightmd` is unavailable, fall back to unscoped `mostlyright-core` / `mostlyright-weather` / `mostlyright-markets` per `.planning/research/TS-SDK-DESIGN.md` §13.1 — this is a P0 fork point.
- **OP4a** (REQUIRED): Register 4 npm OIDC pending publishers on npmjs.com, **bound to `mostlyrightmd/mostlyright-sdk`** (NOT helloiamvu/tradewinds):
  - `@mostlyrightmd/core` → owner `mostlyrightmd`, repo `mostlyright-sdk`, workflow `release-ts.yml`, env `npm`
  - `@mostlyrightmd/weather` → same
  - `@mostlyrightmd/markets` → same
  - `mostlyright` (unscoped meta) → same
- **OP4b** (REQUIRED): Create GH repo Environment `npm` on `mostlyrightmd/mostlyright-sdk`. Set required reviewer = operator.
...
- `scripts/release-ts-preflight.mjs` — preflight that rewrites peerDependencies['@mostlyrightmd/core'] (Phase 12 iter-1 CRITICAL fix)
...
- `npm install @mostlyrightmd/core@next` (smoke install from npm next channel)
- `npm install @mostlyrightmd/core` (smoke install from npm latest)
...
| Req | Wave |
|-----|------|
| NPM-01: `@mostlyrightmd` scope claimed | W1 (operator pre-flight) |
...
| NPM-04: `npm install @mostlyrightmd/core@next` works in clean Node 20 + pnpm 9 project | W2 |
...
1. All 4 npm packages (`@mostlyrightmd/{core,weather,markets}` + unscoped meta `mostlyright`) published at vts-0.1.0 on npm with `latest` dist-tag.
2. `npm install @mostlyrightmd/core` (no @next) in clean Node 20 project resolves to 0.1.0; TS types load via `dist/index.d.ts`; smoke import works.
...
- **npm scope conflict resolution** — if `@mostlyrightmd` is taken, fall back to unscoped `mostlyright-core` / `mostlyright-weather` / `mostlyright-markets`; operator decision at OP3 time.
