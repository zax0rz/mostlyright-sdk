...
### Multi-vertical proof — second-vertical SELECTION is a user-owned decision (researcher overrides initial recommendation)
- **Locked:** v0.2 ships weather (Phase 2 baseline) + ONE new vertical. v0.3 deepens + adds further verticals. >2 verticals in v0.2 = scope explosion (forbidden); zero new verticals = defeats "multi-vertical" success criterion (forbidden).
- **REVISED 2026-05-22 (RESEARCH.md §E):** The initial "sports prediction markets" hint from VISION.md is **NOT viable in 2026** — horse racing was federally blocked from prediction markets in May 2026 (1978 Interstate Horseracing Act litigation outcome); NBA/NFL Kalshi contracts are in active litigation (New Mexico tribes won injunction motion May 2026; NJ appeal pending); MLB signed an exclusive prediction-market deal with Polymarket (March 2026). Sports v0.3 minimum, after 2026-2027 legal landscape settles.
- **Researcher's recommendation (pending user confirmation):** **macroeconomic indicators (FRED + ALFRED → Kalshi CPI/PCE/payrolls/Fed-funds contracts)** as the v0.2 second vertical. Rationale: (a) FRED is free + well-documented; (b) ALFRED's vintage API is a built-in `(event_time, knowledge_time)` primitive — maps 1:1 to our temporal-safety contract; (c) Kalshi macro markets have multi-year operational history; (d) no legal exposure (CFTC-blessed); (e) Fed working paper documents Kalshi macro markets as quant-grade.
- **For planner:** treat this as a USER-OWNED DECISION GATE. Default the plan to macro-indicators (researcher's pick) but include an explicit checkpoint task BEFORE Wave 4 (the vertical-specific wave) where the user confirms macro vs sports vs other. Most of Phase 5 (Waves 1-3) is vertical-agnostic — server skeleton, catalog format, temporal middleware, agent-connector pipeline — so deferring the vertical decision until Wave 4 doesn't block earlier work.
- **Forbid:** silently choosing sports without user override of the legal-blocker finding; shipping no second vertical; shipping >2 verticals.
