...
- **W0 operator pre-flight (NEW — added 2026-05-25):** (a) operator creates `mostlyrightmd` GitHub org, (b) repo cleanup commit on `main` (untrack `.planning/`, prune dev artifacts, write production-grade root README — paired with Phase 16; see X-ref), (c) operator transfers `helloiamvu/tradewinds` → `mostlyrightmd/mostlyright-sdk`. All subsequent waves bind to the **new** repo coordinates.
- 3 PyPI pending-publisher registrations against `mostlyrightmd/mostlyright-sdk` (operator-gated, manual on pypi.org)
- 3 TestPyPI pending-publisher registrations against `mostlyrightmd/mostlyright-sdk` (operator-gated, manual on test.pypi.org)
- GH repo Environments `pypi` + `testpypi` on the NEW repo (operator-gated, GH UI)
- `v0.1.0rc1` git tag pushed to `mostlyrightmd/mostlyright-sdk` → release-testpypi.yml fires → 3 wheels on test.pypi.org
...
- **OP0a** (REQUIRED, ordered first): Operator creates `mostlyrightmd` GitHub organization at https://github.com/organizations/new. Personal account on the org. Org owner = operator. No teams needed for v1.0.
...
- **OP0c** (REQUIRED, ordered third): Operator transfers `helloiamvu/tradewinds` → `mostlyrightmd/mostlyright-sdk` via GitHub Settings → General → Transfer ownership. New repo URL: `https://github.com/mostlyrightmd/mostlyright-sdk`. Local clone `git remote set-url origin git@github.com:mostlyrightmd/mostlyright-sdk.git`. The old URL `helloiamvu/tradewinds` auto-redirects for 1 year per GitHub policy, but planning artifacts in this repo should be updated to the new URL.
...
- **OP2a** (REQUIRED): Register 3 PyPI pending publishers on pypi.org, **bound to `mostlyrightmd/mostlyright-sdk`** (NOT helloiamvu/tradewinds):
  - `mostlyright` → owner `mostlyrightmd`, repo `mostlyright-sdk`, workflow `release.yml`, env `pypi`
...
- **OP2c** (REQUIRED): Create GH repo Environments `pypi` + `testpypi` on `mostlyrightmd/mostlyright-sdk`. Set `pypi` to require reviewer approval; `testpypi` optional.
...
| Req | Wave |
|-----|------|
| PYPI-00: org created + cleanup commit landed + repo transferred to `mostlyrightmd/mostlyright-sdk` | **W0 (operator pre-flight, NEW)** |
| PYPI-01: 3 PyPI publishers registered against `mostlyrightmd/mostlyright-sdk` | W1 (operator pre-flight) |
| PYPI-02: 3 TestPyPI publishers registered against `mostlyrightmd/mostlyright-sdk` | W1 (operator pre-flight) |
