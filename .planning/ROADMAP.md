...
**Goal**: Two paired tracks unblock backend choice for downstream consumers. **PANDAS3** — drop the `pandas<3.0` cap that the Phase 1 parity gate locked in, audit + remediate every datetime/dtype risk site the recon found, re-capture the 5 parity fixtures against pandas 3.x so byte-equivalence holds on both 2.x and 3.x lockfiles. **POLARS** — opt-in `backend="polars"` kwarg on every public DataFrame-returning entry point (`research()` / `research_by_source()` / `polymarket_discover()` / `forecast_nwp()` / `daily_extremes()` rollup), narwhals-mediated internal data layer for the cleanly-portable modules, pandas stays canonical for parity-locked paths. `df.attrs` migration is the load-bearing design move — introduce `TradewindsResult` thin wrapper that carries provenance separate from the frame so both backends preserve source-identity invariants without `.attrs` writes that Polars cannot represent.
...
**Success Criteria** (what must be TRUE):
  1. **Pandas 3 dual-lock parity:** the 5 Phase 1 parity fixtures pass byte-equivalent under BOTH `pandas==2.2.x` and `pandas>=3.0,<4.0` lockfiles; CI matrix runs both. The `pandas<3.0` cap in the 6 extras (`core/parquet`, `core/research`, `weather/parquet`, `weather/nwp`, `markets/parquet`, `markets/polymarket`) is dropped; new floor `pandas>=2.2,<4.0`. Fixture re-capture is documented in `tests/fixtures/parity/README.md` with both-pandas SHAs side-by-side.
  2. **Opt-in polars backend:** `backend: Literal["pandas","polars"]="pandas"` kwarg on `research()`, `research_by_source()`, `polymarket_discover()`, `forecast_nwp()`, and `daily_extremes()`. Default stays pandas. With `backend="polars"`, every adapter returns a Polars DataFrame; row contents are equivalent to pandas (verified by `polars_df.to_pandas().equals(pandas_df)` on the 5 parity fixtures); provenance lives on the wrapping `TradewindsResult` (NOT `df.attrs`, which polars lacks).
  3. **`TradewindsResult` provenance wrapper:** new dataclass at `tradewinds.core.result.TradewindsResult` carries `frame` (pd.DataFrame | pl.DataFrame), `source` (str), `retrieved_at` (datetime), `schema_id` (str | None), `qc` (dict | None), `data_version` (DataVersion | None). All adapter outputs return `TradewindsResult` from the new backend-aware paths; legacy DataFrame-direct returns kept under a deprecation shim for one release cycle. Validator + KnowledgeView + LeakageDetector accept `TradewindsResult` and unwrap.
  4. **Narwhals internal data layer:** the 5 cleanly-portable modules (`transforms.py`, `preprocessing.py`, `qc.crosscheck_iem_ghcnh`, `core/temporal/knowledge_view.py`, `core/formats/{json,csv,toon}.py`) are refactored to call narwhals primitives (`nw.from_native(df)` → operations → `nw.to_native(...)`). Per-backend test matrix verifies identical output. Parity-locked modules (`_internal/_pairs.py`, `core/merge.py`, `core/_climate.py`, validator) stay pandas-only — their lift-pinned status is unchanged.
