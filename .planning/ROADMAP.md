...
- [x] **Phase 11: Live Streaming Iterator** [SHIPPED 2026-05-25] [Python v0.2+, paired TS] - `mostlyright.live.stream()` async generator + `mostlyright.live.latest()` one-shot for single-source live ticker (AWC default 30s polite floor / IEM 60s); intentionally distinct from `research()`'s multi-source database surface (no cache writes, no QC). 5-iter codex + python-architect + ts-architect review loop. Test delta: Python +26, TS +31.
- [x] **Phase 12: Rename `tradewinds` → `mostlyright`** [SHIPPED 2026-05-25] - Mechanical end-to-end rename of every in-repo identifier across both SDKs. 3 PyPI distros (mostlyright / -weather / -markets), 5 npm packages (@mostlyrightmd/{codegen,core,weather,markets} + meta), cache env var TRADEWINDS_CACHE_DIR → MOSTLYRIGHT_CACHE_DIR with one-release back-compat shim, schema $id URL tradewinds.dev → mostlyright.dev. Repo URL `helloiamvu/tradewinds` preserved (operator may rename out-of-band). 4-iter review loop. Zero test regression.
- [ ] **Phase 13: PyPI Publication Pipeline** [PLANNED 2026-05-25] [v0.1.0 → v1.0.0] - Stand up the operator-gated PyPI publish playbook end-to-end against the new `mostlyrightmd/mostlyright-sdk` repo. **Wave 0 pre-flight (operator-gated):** (a) operator creates `mostlyrightmd` GitHub org, (b) cleanup commit on `main` removes `.planning/` from tracking + dev artifacts (paired with Phase 16 W0 — see X-ref), (c) operator transfers `helloiamvu/tradewinds` → `mostlyrightmd/mostlyright-sdk`. Then: register the 3 PyPI trusted-publishers against `mostlyrightmd/mostlyright-sdk` (OP2 from Phase 12 closeout, now bound to new repo), dry-run release-testpypi.yml against `v0.1.0rc1` tag → soak on test.pypi.org, install-from-testpypi smoke test, promote `v0.1.0` tag → production PyPI via release.yml, verify METADATA + Requires-Dist pins via the Phase 4 CI-04 gate, post-publish soak (≥1 week on `pip install mostlyright`). Sequenced first because the npm pipeline's parity-check uses the Python distribution as the source-of-truth schema. ~4-5 days. See `.planning/phases/13-pypi-publication-pipeline/13-PLAN.md`.
- [ ] **Phase 14: npm Publication Pipeline** [PLANNED 2026-05-25] [vts-0.1.0 → vts-1.0.0] - npm OIDC trusted-publisher mirror of Phase 13 against `mostlyrightmd/mostlyright-sdk`: **operator gate (d)** — claim `@mostlyrightmd` npm scope UNDER the new `mostlyrightmd` GitHub org (not under helloiamvu), then register the 4 npm scoped/meta publishers (OP3-OP4 from Phase 12 closeout, bound to new repo), dry-run release-ts.yml with `vts-0.1.0rc1` → soak on `npm install @mostlyrightmd/core@next`, install-from-npm smoke test (Node + Bun + browser via Chrome MV3 extension example), promote `vts-0.1.0` → npm `latest`. Coordinates Changesets fixed-group version bump (lockstep across all 4 packages). Depends on Phase 13 (Wave 0 org + transfer must have landed; Python schemas are the codegen source-of-truth; TS publish must match the published Python schema $id URL). ~4-5 days. See `.planning/phases/14-npm-publication-pipeline/14-PLAN.md`.
- [ ] **Phase 15: Docs Auto-Generation + Landing Site Integration** [PLANNED 2026-05-25] [paired Python + TS] - Auto-generate API reference from source-of-truth — Sphinx + sphinx-autodoc for Python (consumes NumPy-style docstrings in `mostlyright.research`, `mostlyright.live`, `mostlyright.discover`, etc.), TypeDoc for TypeScript (consumes JSDoc on the d.ts surface) — emit MDX with stable anchor IDs, push into `mostly-right-landing/src/content/docs/docs/sdk/` (replacing the current 3 placeholder MDX files) as a release-tag-triggered CI workflow on `mostlyrightmd/mostlyright-sdk`, deploy via Cloudflare Pages. Docs cover: quickstart (Python + TS), full API reference per package, cache-migration guide (already shipped), live-streaming guide (already shipped), back-compat / deprecation timeline. All "Edit on GitHub" and source-link anchors point at `github.com/mostlyrightmd/mostlyright-sdk`. Depends on Phase 13 + Phase 14 (need published-package install commands working). ~5-7 days. See `.planning/phases/15-docs-autogen-landing-site/15-PLAN.md`.
- [ ] **Phase 16: v1.0.0 Production Release** [PLANNED 2026-05-25] [v1.0.0 across Python + TS] - Production hardening + version bump for `mostlyrightmd/mostlyright-sdk` (public). **Cleanup wave (executed in Phase 13 Wave 0 OR re-asserted here if not yet shipped):** remove `.planning/` from tracked sources (kept locally via `.gitignore`), prune dev-only artifacts (stash/, debug scripts), rewrite root README.md as user-facing marketing copy (problem → solution → 60-second quickstart → install → API surface map → links to docs site), polish per-package READMEs, add CHANGELOG.md (per-version sections aggregating phase closeouts), CONTRIBUTING.md, CODE_OF_CONDUCT.md, SECURITY.md (vuln-report process), LICENSE confirmation. Bump all 8 package versions (3 PyPI + 5 npm) from 0.1.0 → 1.0.0 in lockstep, tag `v1.0.0` (Python) + `vts-1.0.0` (TS) — release.yml + release-ts.yml on `mostlyrightmd/mostlyright-sdk` fire automatically, publish to PyPI `latest` + npm `latest`. Add status badges (PyPI, npm, CI, license, codecov) all pointing at new repo URL. External validation: time a new user through the README quickstart end-to-end (<5min target). Depends on Phase 13 + 14 + 15 (0.1.0 must be published + docs live before the 1.0 promotion). ~4-5 days. See `.planning/phases/16-v1.0-production-release/16-PLAN.md`.
...
### Phase 6: Pandas 3 Readiness + Optional Polars Backend
**Goal**: Two paired tracks unblock backend choice for downstream consumers. **PANDAS3** — drop the `pandas<3.0` cap that the Phase 1 parity gate locked in, audit + remediate every datetime/dtype risk site the recon found, prove parity under pandas 3.x via the `tests/fixtures/parity/coerce_pd3.py` invertible-transform bridge (NOT a second fixture set — see PLAN W1-T5 for the rationale). **POLARS** — opt-in `backend="polars"` kwarg on every public DataFrame-returning entry point (`research()` / `research_by_source()` / `polymarket_discover()` / `forecast_nwp()` / `daily_extremes()` rollup), narwhals-mediated internal data layer for the cleanly-portable modules (serving DIRECT user calls like `tw.transforms.lag(my_polars_df)` — research() itself stays pandas end-to-end per W4-T1), pandas stays canonical for parity-locked paths. `df.attrs` migration is the load-bearing design move — introduce `TradewindsResult` thin wrapper that carries provenance separate from the frame so both backends preserve source-identity invariants without `.attrs` writes that Polars cannot represent.
**Depends on**: Phase 4 (CI/CD trusted publishing carries forward; parity gate re-runs against pandas 3 fixtures). Independent of Phase 5 (MCP doesn't dictate backend — MCP tools serialize via existing format wrappers regardless of source backend).
**Requirements**: PANDAS3-01..PANDAS3-06 + POLARS-01..POLARS-08 (see REQUIREMENTS.md § Phase 6: Pandas 3 + Polars).
**Success Criteria** (what must be TRUE):
  1. **Pandas 3 dual-lock parity (via coerce_pd3 bridge — NOT a second fixture set):** the 5 Phase 1 parity fixtures pass byte-equivalent under BOTH `pandas==2.2.x` and `pandas>=3.0,<4.0` lockfiles via the `tests/fixtures/parity/coerce_pd3.py` invertible-transform bridge (the canonical 2.x fixtures stay immutable per v0.1.0 contract; pandas-3 test reads them, applies the documented `ns→us` + `object→string` coercion, compares against live `research()` output). CI matrix runs both. The `pandas<3.0` cap in the 6 extras is dropped; new floor `pandas>=2.2,<4.0`. ULP-drift measurement (`tests/fixtures/parity/ulp_drift_pd3.json` — committed artifact) carries the existing `atol=1e-12` ladder forward; promoted to `atol=1e-10` if measured drift exceeds.
  2. **Opt-in polars backend:** `backend: Literal["pandas","polars"]="pandas"` kwarg on `research()`, `research_by_source()`, `polymarket_discover()`, `forecast_nwp()`, and `daily_extremes()`. Default stays pandas. With `backend="polars"` (which requires `return_type="wrapper"` since polars frames have no `df.attrs`), every adapter returns a `TradewindsResult` wrapping a Polars DataFrame; row contents equivalent to pandas (verified by `polars_df.to_pandas().equals(pandas_df)` on the 5 parity fixtures); provenance lives on the wrapper.
  3. **`TradewindsResult` provenance wrapper:** new dataclass at `tradewinds.core.result.TradewindsResult` carries `frame` (pd.DataFrame | pl.DataFrame), `source` (str), `retrieved_at` (datetime), `schema_id` (str | None), `qc` (dict | None), `data_version` (DataVersion | None). `return_type="dataframe"` (default; preserves v0.1.0 shape) keeps existing `df = research(...)["temp"]` callers working; `return_type="wrapper"` is opt-in (required for `backend="polars"`). `daily_extremes()` keeps its v0.1.0 `list[dict]` default; wrapper is opt-in (codex iter-3 P2 fix). Validator + KnowledgeView + LeakageDetector accept `TradewindsResult` and unwrap.
  4. **Narwhals internal data layer for DIRECT user calls:** the 5 cleanly-portable modules (`transforms.py`, `preprocessing.py`, `qc.crosscheck_iem_ghcnh`, `core/temporal/knowledge_view.py`, `core/formats/{json,csv,toon}.py`) accept pandas OR polars via narwhals. These narwhals paths fire when a user calls them **directly** (e.g. `tw.transforms.lag(my_polars_df, "temp")` after their own data prep) — they do NOT fire inside `research()` (which stays pandas end-to-end per W4-T1). Parity-locked modules stay pandas-only. The scope is justified by CONTEXT "baseline quant ergonomics" — quants who already use polars for their main analytics pipeline can call tradewinds transforms on polars frames without converting.
  5. **Optional extras:** new `[polars]` extra on all three packages (`tradewinds[polars]`, `tradewinds-weather[polars]`, `tradewinds-markets[polars]`) adds `polars>=1.0,<2.0` + `narwhals>=1.20,<2.0`. Default install never pulls polars; calling a public surface with `backend="polars"` without the extra raises `SourceUnavailableError` (matching the `[nwp]` pattern) with install hint. CI matrix runs the cross-backend tests gated on the extra being installed.
**Out of scope** (deferred to v0.2.x / v0.3+):
- pyarrow Table backend (third option) — narwhals supports it but adds another matrix dimension; v0.3 if user demand emerges.
- Polars LazyFrame default — v0.2 ships eager Polars only; lazy planning is an opt-in extension.
- Backend autodetection from environment — explicit `backend=` kwarg is the only contract in v0.2.
- Rewriting parity-locked paths in polars — `_pairs.py`, `core/merge.py`, `_climate.py`, validator stay pandas; switching them would invalidate the lift-pinned SHA contract per CLAUDE.md.
- `df.attrs` deprecation enforcement — v0.2 supports BOTH `df.attrs` reads (legacy) and `TradewindsResult` (new); strict deprecation lands v0.3.
**Review panel**: Standard 2-reviewer (codex `high` + python-architect). The pandas 3 fixture re-capture is the parity-adjacent gate (must not silently shift bytes); the polars backend has a narwhals-API surface that needs scrutiny for "does narwhals actually preserve sort-stability across backends?" (the parity-gate's `kind="mergesort"` invariant). Hypothesis property tests for `polars_df.to_pandas().equals(pandas_df)` across all 5 fixtures.
**Plans**: TBD (run `/gsd-plan-phase 06` to break down — directory: `.planning/phases/06-pandas-3-polars-backend/`).
