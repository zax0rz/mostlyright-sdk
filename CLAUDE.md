...
| **filelock** | 3.29.0 (Apr 2026) | **`>=3.20,<4`** | Cross-process lock for `$HOME/.mostlyright/cache/.../month.parquet` writes | Battle-tested in pip/uv/tox internals. The 3.29 release improved Windows stale-lock detection — worth picking up the floor bump. |
...
| **pytest-cov** | 6.x | `>=6.0,<8` | Coverage reporting → CI gate | Branch coverage on `mostlyright.core` (≥90% per ROADMAP). |
...
| **mypy** | 1.18.x (Apr 2026) | `>=1.18,<2` | Static type checking on `mostlyright.core` public surface | See §Type checking strategy — **strict on `core/`, lax on `_vendor/`**. |
...
| Tool | Approach | Verdict for mostlyright |
|------|----------|-----------------------|
...
# Strict on mostlyright.core only (the load-bearing safety layer)
...
|-------------|-------------|-------------------------|
| `httpx` | `requests` | Sync-only legacy code. mostlyright adapters might go async in v0.2 (parallel adapter fetches) — staying on httpx now is path-of-least-resistance for that future. |
...
- Document constraint: mostlyright catalog adapters use **buffered httpx responses only**, no streaming.
...
- All three packages can be merged into one without API change — they share the `mostlyright.*` namespace already.
