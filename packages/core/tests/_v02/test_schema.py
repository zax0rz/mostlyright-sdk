...
class TestColumnSpecConstruction:
    def test_minimal_string_column(self) -> None:
        spec = ColumnSpec(name="station", dtype="string", units=None, nullable=False)
    ...
    def test_frozen_dataclass(self) -> None:
        spec = ColumnSpec(name="x", dtype="string", units=None, nullable=False)
...
class TestColumnNames:
    ...
    def test_imperial_mode_with_no_renames_equals_metric(self) -> None:
        assert _ToySchemaNoRenames.column_names("imperial") == _ToySchemaNoRenames.column_names(
            "metric"
        )
...
class TestSchemaRegister:
    def test_register_returns_registration(self) -> None:
        reg = _ToySchema.register(source="iem.archive", retrieved_at=_TS, rows=42)
    ...
    def test_register_timestamp_is_tz_aware(self) -> None:
        reg = _ToySchema.register(source="awc.live", retrieved_at=_TS, rows=1)
    ...
    def test_register_rejects_naive_datetime(self) -> None:
        ...
        with pytest.raises(ValueError, match="timezone-aware"):
            _ToySchema.register(source="iem.archive", retrieved_at=naive, rows=1)
    ...
    def test_register_accepts_non_utc_tz(self) -> None:
        ...
        reg = _ToySchema.register(source="iem.archive", retrieved_at=ts_et, rows=1)
    ...
    def test_register_normalises_retrieved_at_to_utc(self) -> None:
        ...
        assert reg.retrieved_at_min == datetime(2026, 5, 21, 12, 0, 0, tzinfo=UTC)
    ...
    def test_register_rejects_negative_rows(self) -> None:
        with pytest.raises(ValueError, match="rows must be >= 0"):
            _ToySchema.register(source="iem.archive", retrieved_at=_TS, rows=-1)
    ...
    def test_register_accepts_zero_rows(self) -> None:
        reg = _ToySchema.register(source="iem.archive", retrieved_at=_TS, rows=0)
        assert reg.rows == 0
...
class TestAuditLog:
    def _fresh_registration(self) -> SchemaRegistration:
        return _ToySchema.register(source="iem.archive", retrieved_at=_TS, rows=10)
    ...
    def test_append_with_explicit_ts_string(self) -> None:
        reg = self._fresh_registration()
        reg._append_audit("source_drift_allowed", ts="2026-06-01T09:30:00Z", reason="r")
