...
class ColumnSpec:
    ...
    def __post_init__(self) -> None:
        ...
        if self.dtype == "enum" and not self.enum_values:
            raise ValueError(
                f"ColumnSpec {self.name!r}: dtype='enum' requires non-empty "
                f"enum_values"
            )
...
def _require_tz_aware(ts: datetime, field_name: str) -> datetime:
    ...
    if not isinstance(ts, datetime):
        raise TypeError(
            f"{field_name} must be a datetime, got {type(ts).__name__}"
        )
...
class Schema:
    ...
    def column_names(cls, mode: str = "metric") -> list[str]:
        ...
        raise ValueError(
            f"mode must be 'metric' or 'imperial' (got {mode!r})"
        )
    ...
    def from_dataframe(
        ...
        raise NotImplementedError(
            "Schema.from_dataframe is deferred to v0.1.1; "
            "use declarative subclasses for v0.1.0."
        )
