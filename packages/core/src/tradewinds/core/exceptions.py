...
class LiveStreamError(TradewindsError):
    ...
    only raised by the one-shot :func:`tradewinds.live.latest` surface;
    :func:`tradewinds.live.stream` swallows empty-tick errors and waits
...
class NoLiveDataError(LiveStreamError):
    """:func:`tradewinds.live.latest` returned no observations for the station.
