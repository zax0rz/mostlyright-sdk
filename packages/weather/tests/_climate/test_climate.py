...
the ingest/storage layer that has not been ported to tradewinds yet.

Namespace rewrites:
- ``from mostlyright.weather._climate`` -> ``from tradewinds.weather._climate``
- ``from mostlyright._capabilities import SPECS_DIR`` ->
  ``from tradewinds._internal._capabilities import SPECS_DIR``
...
class TestReportTypePriority:
    def test_priority_values(self):
        from tradewinds.weather._climate import REPORT_TYPE_PRIORITY
...
class TestInferReportType:
    ...
    def test_final_next_day_0620_utc(self):
        ...
        from tradewinds.weather._climate import infer_report_type
...
class TestParseCliRecord:
    def test_valid_record(self):
        ...
        from tradewinds.weather._climate import parse_cli_record
...
class TestLiveIEMCli:
    ...
    these literals with the constant from ``tradewinds.weather._fetchers.climate``.
    """
