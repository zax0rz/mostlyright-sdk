...
def test_iem_archive_source_no_longer_raises_not_implemented() -> None:
    ...
    with patch(
        ...
        adapter = IEMAdapter()
        df = adapter.fetch_forecasts(
            "iem.archive", "KNYC", "2026-05-01", "2026-05-07"
        )
        assert isinstance(df, pd.DataFrame)
        # Verify fetch_iem_mos was called with the right args (model="nbe").
        mock_fetch.assert_called_once_with(
            "KNYC", "2026-05-01", "2026-05-07", model="nbe"
        )
...
def test_iem_live_source_raises_not_implemented_with_v02_hint() -> None:
    ...
    with pytest.raises(NotImplementedError, match="iem.live MOS deferred to v0.2"):
        adapter.fetch_forecasts(
            "iem.live", "KNYC", "2026-05-01", "2026-05-07"
        )
...
def test_iem_unknown_source_raises_value_error() -> None:
    ...
    with pytest.raises(ValueError, match="source must be one of"):
        adapter.fetch_forecasts(
            "ghcnh.archive", "KNYC", "2026-05-01", "2026-05-07"
        )
